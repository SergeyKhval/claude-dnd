# D&D 5e Campaign Engine

This project is a D&D 5e game engine plugin for Claude Code. All game state lives in markdown files under `game/`.

## Skills

- `skills/dm/` — Dungeon Master engine for running D&D 5e campaigns

## Commands

- `commands/new-campaign.md` — Create a new campaign
- `commands/start-session.md` — Resume a campaign session
- `commands/save.md` — Save current progress
- `commands/end-session.md` — End the session and save
